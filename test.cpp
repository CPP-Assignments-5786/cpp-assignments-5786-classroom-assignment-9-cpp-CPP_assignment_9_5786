...
TEST_CASE("Riddle 1: twoSum") {
    auto [i, j] = twoSum({2, 7, 11, 15}, 9);
    CHECK(i == 0);
    CHECK(j == 1);
}

TEST_CASE("Riddle 1: twoSum - later indices") {
    auto [i, j] = twoSum({1, 2, 3, 4, 5}, 9);
    CHECK(i == 3);
    CHECK(j == 4);
}

TEST_CASE("Riddle 1: twoSum - no solution throws") {
    CHECK_THROWS_AS(twoSum({1, 2, 3}, 100), std::invalid_argument);
}

TEST_CASE("Riddle 2: firstUniqueChar") {
    CHECK(firstUniqueChar("leetcode") == 'l');
    CHECK(firstUniqueChar("aabb") == '\0');
    CHECK(firstUniqueChar("abcabc") == '\0');
    CHECK(firstUniqueChar("abcab") == 'c');
    CHECK(firstUniqueChar("") == '\0');
    CHECK(firstUniqueChar("z") == 'z');
}

TEST_CASE("Riddle 3: areAnagrams") {
    CHECK(areAnagrams("listen", "silent") == true);
    CHECK(areAnagrams("hello", "world") == false);
    CHECK(areAnagrams("Astronomer", "Moon starer") == true);
    CHECK(areAnagrams("", "") == true);
    CHECK(areAnagrams("abc", "ab") == false);
}

TEST_CASE("Riddle 4: longestConsecutive") {
    CHECK(longestConsecutive({100, 4, 200, 1, 3, 2}) == 4);
    CHECK(longestConsecutive({0, 3, 7, 2, 5, 8, 4, 6, 0, 1}) == 9);
    CHECK(longestConsecutive({}) == 0);
    CHECK(longestConsecutive({1}) == 1);
    CHECK(longestConsecutive({1, 1, 1}) == 1);
}

TEST_CASE("Riddle 5: groupAnagrams") {
    ...
    for (const auto& g : groups) {
        totalWords += static_cast<int>(g.size());
    }
    CHECK(totalWords == 6);
}

TEST_CASE("Riddle 5: groupAnagrams - empty") {
    auto groups = groupAnagrams({});
    CHECK(groups.empty());
}

TEST_CASE("Riddle 5: groupAnagrams - no anagrams") {
    auto groups = groupAnagrams({"abc", "def", "ghi"});
    CHECK(groups.size() == 3);
}

TEST_CASE("Riddle 6: mostFrequent") {
    CHECK(mostFrequent({1, 3, 2, 3, 1, 3, 2}) == 3);
    CHECK(mostFrequent({5}) == 5);
    CHECK(mostFrequent({1, 1, 2, 2}) == 1);  // tie → first appearing
}

TEST_CASE("Riddle 6: mostFrequent - empty throws") {
    CHECK_THROWS_AS(mostFrequent({}), std::invalid_argument);
}

TEST_CASE("Riddle 7: removeDuplicates") {
    auto result = removeDuplicates({3, 1, 4, 1, 5, 9, 2, 6, 5, 3});
    CHECK(result.size() == 7);
    CHECK(result[0] == 3);
    CHECK(result[1] == 1);
    CHECK(result[2] == 4);
...
TEST_CASE("Riddle 7: removeDuplicates - empty") {
    auto result = removeDuplicates({});
    CHECK(result.empty());
}

TEST_CASE("Riddle 7: removeDuplicates - no duplicates") {
    auto result = removeDuplicates({1, 2, 3});
    CHECK(result.size() == 3);
}