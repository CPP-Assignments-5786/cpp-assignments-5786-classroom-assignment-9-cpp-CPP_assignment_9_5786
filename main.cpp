 ...
 *
 * This file demonstrates STL containers, tuples, strings,
 * type traits, and riddle solutions.
 */
...
int main() {
    ...
    std::cout << "twoSum({2,7,11,15}, 9): indices " << i << ", " << j << "\n";
    // Expected: twoSum({2,7,11,15}, 9): indices 0, 1

    // Riddle 2: First Unique Char
    std::cout << "firstUniqueChar('leetcode'): " << firstUniqueChar("leetcode") << "\n";
    // Expected: firstUniqueChar('leetcode'): l

    std::cout << "firstUniqueChar('aabb'): '"
              << static_cast<int>(firstUniqueChar("aabb")) << "'\n";
    // Expected: firstUniqueChar('aabb'): '0'  (null char '\0' cast to int prints 0)

    // Riddle 3: Anagrams
    std::cout << "areAnagrams('listen', 'silent'): "
              << areAnagrams("listen", "silent") << "\n";
    // Expected: areAnagrams('listen', 'silent'): 1

    std::cout << "areAnagrams('hello', 'world'): "
              << areAnagrams("hello", "world") << "\n";
    // Expected: areAnagrams('hello', 'world'): 0

    // Riddle 4: Longest Consecutive
    std::vector<int> seq = {100, 4, 200, 1, 3, 2};
    std::cout << "longestConsecutive: " << longestConsecutive(seq) << "\n";
    // Expected: longestConsecutive: 4

    // Riddle 5: Group Anagrams
    std::vector<std::string> words = {"eat", "tea", "tan", "ate", "nat", "bat"};
    auto groups = groupAnagrams(words);
    std::cout << "groupAnagrams: " << groups.size() << " groups\n";
    // Expected: groupAnagrams: 3 groups

    // Riddle 6: Most Frequent
    std::vector<int> freqNums = {1, 3, 2, 3, 1, 3, 2};
    std::cout << "mostFrequent: " << mostFrequent(freqNums) << "\n";
    // Expected: mostFrequent: 3

    // Riddle 7: Remove Duplicates
    std::vector<int> dups = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3};
    auto unique = removeDuplicates(dups);
    std::cout << "removeDuplicates: ";
    for (int n : unique) {
        std::cout << n << " ";
    }
    std::cout << "\n";
    // Expected: removeDuplicates: 3 1 4 5 9 2 6
